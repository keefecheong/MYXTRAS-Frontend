    ...
    <div id="main-container">
        ...
        <div id="main-content" class="gachaponBody">
            <button class="toggle-button" @click="toggleBetween">
                {{ viewingGacha ? "View Inventory" : "Gachapon" }}
            </button>
            <Pet />
            <div id="left-content">
                <div class="wallet-container">
                    ...
                    <h5>
                        {{ this.gems }}
                        <span class="material-symbols-outlined gemIcon"
                            >diamond</span
                        >
                        <span v-if="deduction" style="color: red; margin: 0.5vw"
                            >-{{ deduction }}</span
                        >
                    </h5>
                </div>
                <h2 class="header">Your Pets Collection:</h2>
                <div id="white-container">
                    <p v-if="inventoryPets.length == 0" id="no-pets-text">
                        No pets in inventory!
                    </p>
                    <ul v-for="pet in inventoryPets">
                        <div
                            id="pet-list-container"
                            :class="{
                                'selected-pet': pet.name === selectedPet,
                            }"
                        >
                            <li
                                id="petsList"
                            ...
                            <li
                                ...
                                :style="getRarityStyle(pet.rarity)"
                            >
                                {{ pet.rarity }}
                            </li>
                        </div>
                ...
                <div class="settings-container">
                    <span id="enable-title" class="image-options-label"
                        >Enable Pets:</span
                    >
            ...
            <div id="right-content">
                ...
                <div id="buttons-container">
                    <button id="spinOneBtn" @click="rollGacha(1)">
                        Spin 1
                    </button>
                    <button id="spinTenBtn" @click="rollGacha(10)">
                        Spin 10
                    </button>
                </div>
                <div id="buttons-container">
            ...
            <v-dialog v-model="showModal" max-width="500">
                <v-card class="v-theme--dark">
                    <v-card-title v-if="!insufficientGems"
                        >Rolled Pets</v-card-title
                    >
                    <v-card-text>
                        <!-- Display the rolledPets data here -->
                        <ul v-if="!insufficientGems">
                            <li
                                v-for="pet in rolledPets"
                                :key="pet.name"
                                class="listOptions"
                                style="color: white"
                            >
                                ...
                                <span style="float: right" v-if="!pet.new">
                                    Refunded 80
                                    <span
                                        class="material-symbols-outlined gemIcon"
                                        >diamond</span
                                    >
                                </span>
                                <p
                        ...
                        <p v-if="insufficientGems">
                            Insufficient gems! Need {{ this.gemsReq }} more
                            gems.
                        </p>
                    </v-card-text>
                    <v-card-actions>
            ...
            await fetch(
                ...
                .then(async (res) => {
                    await res.json().then((data) => {
                        ...
                        this.inventoryPets = this.inventoryPets.filter(
                            (obj, index, arr) => {
                                return (
                                    arr.findIndex(
