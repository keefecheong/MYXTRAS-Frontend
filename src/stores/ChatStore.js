    ...
    actions: {
        ...
        newChat(chat) {
            ...
            if (!existingChat) {
                ...
                const currentTimestamp = new Date(
                    existingChat.last_message_timestamp,
                );
        ...
        newChatBulk(chats) {
            ...
            chats.forEach((chat) => {
                ...
                const chatDiff = existingChats.some(
                    (existingChat) =>
        ...
        newMessage(message) {
            ...
            if (
                !this.messages[message.chat_id].some(
                    (addedMessage) => addedMessage._id == message._id,
                )
            ) {
        ...
        newMessageBulk(messages, chatId) {
            ...
            const nonConflictingMessages = messages.filter(
                (message) =>
                    !this.messages[chatId].some(
                        (existingMessage) => existingMessage._id == message._id,
                    ),
            );
            this.messages[chatId] = this.messages[chatId].concat(
                nonConflictingMessages,
            );
