        ...
        <div class="detailed-layout-content-container">
            ...
            <div>
                ...
                <div v-if="admin">
                    <span>Object ID:</span>
                    <span>{{
                        aggregated ? report._id : report.report_target
                    }}</span>
                </div>
            ...
            <div>
                ...
                <div v-if="admin">
                    ...
                    <span
                        ...
                        @click="() => viewUser(report.report_target_owner._id)"
                        >{{ report.report_target_owner._id }}</span
                    >
                </div>
            </div>
            ...
            <div>
                ...
                <div>
                    ...
                    <ul v-if="aggregated">
                        <li v-for="reason in report.report_reasons">
                            {{ reason }}
                        </li>
                    </ul>
                ...
                <div
                    v-if="
                        report.report_reasons?.includes('Other') ||
                        report.report_reason == 'Other'
                    "
                >
                    <span>Supporting Description(s):</span>

                    <ul v-if="aggregated">
                        <li v-for="reason in report.report_other_reasons">
                            {{ reason }}
                        </li>
                    </ul>
                ...
                <div
                    v-if="
                        report.report_target_type == REPORT_TARGET_TYPE_USER &&
            ...
            <div>
                ...
                <div v-if="reviewed && admin">
                    ...
                    <span
                        class="details-layout-profile-link"
                        title="Click to view user"
                        @click="() => viewUser(report.reviewer_id._id)"
                        >{{ report.reviewer_id._id }}</span
                    >
                </div>
            ...
            <div
                id="detailed-report-view-messages-container"
                v-if="viewMessages"
            >
                <ChatMessageLayout
...
export default {
    ...
    props: ["report", "aggregated", "admin"],
    emits: ["close-report-details"],
    mounted() {
        window.scrollTo(0, 0);
    },
    computed: {
        ...
        viewTitle() {
            return this.report.report_target_type ==
                this.REPORT_TARGET_TYPE_USER
                ? "View User"
                : "View Content";
        },
        ...
        reportParent() {
            switch (this.report.report_target_type) {
                case this.REPORT_TARGET_TYPE_COMMENT:
                    return {
                        type: this.report.meta.comment_parent_type,
                        id:
                            this.report.meta.post_id ||
                            this.report.meta.thread_id,
                    };
        ...
        resolveReportType() {
            if (
                this.report.report_target_type !=
                this.REPORT_TARGET_TYPE_COMMENT
            )
                return this.report.report_target_type;

            return this.report.meta.comment_parent_type ==
                this.REPORT_TARGET_TYPE_POST
                ? this.REPORT_TARGET_TYPE_POST_COMMENT
                : this.REPORT_TARGET_TYPE_THREAD_COMMENT;
        },
        ...
        resolveUserId() {
            if (
                ![
                    ...
                    this.REPORT_TARGET_TYPE_POST,
                    this.REPORT_TARGET_TYPE_POST_COMMENT,
                ].includes(this.resolveReportType)
            )
                return null;
        ...
        resolveForumId() {
            if (
                ![
                    this.REPORT_TARGET_TYPE_FORUM,
                    this.REPORT_TARGET_TYPE_THREAD,
                    this.REPORT_TARGET_TYPE_THREAD_COMMENT,
                ].includes(this.resolveReportType)
            )
                return null;
        ...
        resolveCommentId() {
            return this.report.report_target_type ==
                this.REPORT_TARGET_TYPE_COMMENT
                ? this.report.report_target
                : null;
        },
        // get messageId to pass to reportFormLayout
        resolveMessageId() {
            return this.report.report_target_type ==
                this.REPORT_TARGET_TYPE_MESSAGE
                ? this.report.report_target
                : null;
        },
    },
                ...
                case this.REPORT_TARGET_TYPE_MESSAGE:
                    if (this.messages.length <= 0) {
                        ...
                        await fetch(
                            ...
                            }/messages/${this.report.report_target}`,
                            {
                                mode: "cors",
                                method: "GET",
                                credentials: "include",
