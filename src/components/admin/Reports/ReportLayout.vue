        ...
        >
            <div>
                ...
                <span
                    ...
                    v-if="!fromProfile"
                    :title="displayTarget[i]"
                    >{{ displayTarget[i] }}</span
                >

                <span
                    class="hide-overflow-text flex-10"
                    v-if="aggregated && !reviewed"
                    :title="displayCount[i]"
                    >{{ displayCount[i] }}</span
                >

                <span
                    class="hide-overflow-text flex-10"
                    :title="displayOwner[i]"
                    >{{ displayOwner[i] }}</span
                >

                <span
                    class="hide-overflow-text flex-10"
                    v-if="!reviewed && !fromProfile"
                    :title="displayReporter[i]"
                    >{{ displayReporter[i] }}</span
                >

                <span
                    class="hide-overflow-text flex-15"
                    v-if="!reviewed"
                    :title="displayReportTime[i]"
                    >{{ displayReportTime[i] }}</span
                >
                ...
                <span
                    class="hide-overflow-text flex-10"
                    v-if="reviewed"
                    :title="displayReviewer[i]"
                    >{{ displayReviewer[i] }}</span
                >

                <span
                    class="hide-overflow-text flex-15"
                    v-if="reviewed"
                    :title="displayReviewTime[i]"
                    >{{ displayReviewTime[i] }}</span
                >
            </div>

            <button
                class="details-button use-primary-secondary-gradient"
...
export default {
    props: ["report", "aggregated", "fromProfile", "index"],
    emits: ["view-report-details"],
    computed: {
